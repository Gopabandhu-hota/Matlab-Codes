% This is for non-ideal load with all parrallel elements
RL = 1000;
RS = 50;
f = 10^5;
W = 2*pi*f;
...
for k = 1:size(w,2)
   ...
   Power(1,k) = abs(V*XC_LC*Z/((RS+XL_LC+Znet_LC)*(XC_LC+Z)))^2/RL;   %Power of LC  low pass Network 
   Power(2,k)= abs(V*XL_CL*Z/((RS+XC_CL+Znet_CL)*(XL_CL+Z)))^2/RL;   %CL high pass Network 
end
...
title('Power variation of non-ideal parralel load with frequency for both LC and CL network');
